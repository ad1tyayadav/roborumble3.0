...
export default function GalleryPage() {
  ...
  return (
    ...
    >
      ...
      {/* --- Marquee Top --- */}
      <div className="fixed top-20 left-0 w-full bg-[#FF003C] z-10 overflow-hidden py-1 rotate-1 shadow-[0_0_20px_#FF003C]">
        <motion.div 
            className="flex whitespace-nowrap gap-8"
            animate={{ x: "-100%" }}
            transition={{ duration: 20, repeat: Infinity, ease: "linear" }}
        >
          {Array(10).fill("MEMOIRS OF THE PAST * ROBO RUMBLE * 2025 * ").map((text, i) => (
             <span key={i} className="text-black font-black italic tracking-widest text-sm md:text-base font-['Orbitron']">
               {text}
             </span> 
          ))}
        </motion.div>
      ...
      <div className="pt-40 px-4 md:px-8 max-w-[1600px] mx-auto relative cursor-crosshair">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-12 relative z-1 min-h-[150vh] pb-40">
           ...
           <motion.div style={{ y: y1 }} className="flex flex-col gap-12 mt-0">
              {columns[0].map((src, i) => (
                <GalleryItem key={i} src={src} index={i} onClick={() => setSelectedImage(src)} />
              ))}
           </motion.div>

           {/* Column 2 (Offset start) */}
           <motion.div style={{ y: y2 }} className="flex flex-col gap-12 mt-24 md:mt-60">
              {columns[1].map((src, i) => (
                <GalleryItem key={i} src={src} index={i} onClick={() => setSelectedImage(src)} />
              ))}
           </motion.div>
           ...
           {/* Column 3 */}
           <motion.div style={{ y: y3 }} className="flex flex-col gap-12 mt-12 md:mt-20">
              {columns[2].map((src, i) => (
                <GalleryItem key={i} src={src} index={i} onClick={() => setSelectedImage(src)} />
              ))}
           </motion.div>
...
const GalleryItem = ({ src, index, onClick }: { src: string; index: number; onClick: () => void }) => {
